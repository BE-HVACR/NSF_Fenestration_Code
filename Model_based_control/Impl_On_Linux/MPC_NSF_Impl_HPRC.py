...
def preProc_ZoneDyn(dat_log,dat_out,dat_wea):
    ...
    ## rename columns
    dat_feature.columns = ['EC_V', 'B_angle','B_Height', 
                        'SA Mass Flow Rate',
...
def run_prediction(x,solution_idx,hyperParam):
    ...
    temp_dict_ZoneDyn['B_angle']=x[0+hyperParam['PH']:0+hyperParam['PH']*2]
    temp_dict_ZoneDyn['B_Height']=x[0+hyperParam['PH']*2:0+hyperParam['PH']*3]
    ...
    DF_lightEst['angle'] = x[0+hyperParam['PH']:0+hyperParam['PH']*2]
    height_temp = x[0+hyperParam['PH']*2:0+hyperParam['PH']*3]
...
def read_Glazing_txt(fileName):
    ## read 
    re_Array = []
    with open(r"./examples/GlazingData"+"/" + fileName) as fp:
        count  = 0
...
def geneGlazingFile(voltage_input):
    if voltage_input >4 or voltage_input <0:
        ...
        # write data
        with open(r"./examples/GlazingData/temp.dat",'w+') as fp:
            fp.writelines(lines)
    return lines
...
def calculate_BSDF(EC_voltage,blind_height,bling_angle):
    ...
    optical_standard_path = r"./examples/standards/W5_NFRC_2003.std"
    ...
    if bling_angle>=0:
        bsdf_shade = pywincalc.parse_bsdf_xml_file(r"./examples/tmx/blinds_Angle{}.xml".format(bling_angle))
    else:                                            
        bsdf_shade = pywincalc.parse_bsdf_xml_file(r"./examples/tmx/blinds_Angle_N{}.xml".format(0-bling_angle))
    
    
    ## Generate glazing data file for Voltage to temp.dat
    geneGlazingFile(EC_voltage) 
    
    clear_3_path = r"./examples/GlazingData/temp.dat"
...
if __name__ == "__main__":
    ...
    while True:
        ...
        api = "2"
        fmu_path = r'EplusModel\in_WithCFS.fmu'
        model="model_1_"+api
        ...
        warmup_time = start_time + 60*60*24 
        while tim < final_time:
            ...
            if tim < warmup_time:
                ...
                res = fitness_function([2.5,0,5,350,0.25,14,0],0,hyperParam)
                ...
                #write results
                EC_voltage = controlVariable[0]
                blind_Angle = controlVariable[1]
                blind_height = controlVariable[2]
            ...
            # Save the operational data at this timestep. 
            hourOfYear = int(hyperParam['tim']/3600)
            temp_dict_ZoneDyn = {}
            dat_Op = pd.read_csv(r'./Data/OperationDat.csv')
            ...
            temp_dict_ZoneDyn['B_angle']=[controlVariable[1]]
            temp_dict_ZoneDyn['B_Height']=[controlVariable[2]]
