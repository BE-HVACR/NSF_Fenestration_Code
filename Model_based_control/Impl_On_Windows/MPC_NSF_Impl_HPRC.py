...
if __name__ == "__main__":
    ...
    while True:
        ...
        api = "2"
        fmu_path = r'in_WithCFS_withFMI.fmu'
        model="model_1_"+api
        ...
        warmup_time = start_time + 60*60*36 
        while tim < final_time:
            ...
            if tim < warmup_time:
                ...
                num_genes = hyperParam['PH']*3

                # define the gene spaces
                res = fitness_function([350,0.25,14,0],0,hyperParam)
                #[2.5,0,5,350,0.25,14,0]  
                gene_space =[{'low':0,'high':400}] * hyperParam['PH']+[{'low':0.35,'high':0.35}] * hyperParam['PH'] +[{'low':12,'high':18}] * hyperParam['PH']
                ...
                controlVariable = [4,90,100]+list(re[0][0])+[1] # Turn on control 
                ...
                '''
                re = [400,0.25,14]
                controlVariable = [4,90,100]+re+[1] # Turn on control 
                '''
                
                #write results
                EC_voltage = controlVariable[0]
                blind_height = controlVariable[1]
                blind_Angle = controlVariable[2]


                temp_lines = []
                temp_lines.append(",".join([str(x) for x in controlVariable]) + "\n")
                with open(r'ControlVariables.txt','w') as fp:
                    fp.writelines(temp_lines)
